...
var glicko = module.exports = {};

//wrapper to make it easier to use as a node.js module
glicko.playerInit = function(player){
	if(!player.rating)
		player.rating = 1500;
	if(!player.rd)
		player.rd = 350;
...
exports.calc = function(player, opponents, rating_period, system_constant, factor){
	...
	var initialize_set = [];
	async.forEach(opponents, function(opp){
		initialize_set.push(opp);
	});

	initialize_set.push(p);
	async.forEach(initialize_set, function(pl) {
	    //step 2 - set glicko-2 values (and initial values).
	    glicko.playerInit(pl);
	});
	...
	var E = function(p1, p2){
		if(!p1.rd2)
			throw "Missing player 1" + p1;
		if(!p2.rd2)
			throw "Missing player 2"+ p2;
...
returns {
        ...
        */
glicko.teamMatch = function(teams, rating_period, system_constant) {
	var players = [];
	async.forEach(teams, function(team, t1){
		async.forEach(team.players, (function(p){
			players.push(p);
			...
			var opponents = [];
			//round up opponents
			async.forEach(teams, function(team2, t2){
				if(t2==t1)
				...
				async.forEach(team2.players, function(p2){
					glicko.playerInit(p2);
			    	async.forEach(Object.keys(team_as_player), function(item, callback){
						team_as_player[item] += p2[item];
					});
				});
			    ...
			    async.forEach(Object.keys(team_as_player), function(item, callback){
			    	team_as_player[item] = team_as_player[item]/team2.players.length;
			    });
	...
	async.forEach(players, function(p){
		var update = p.last_result.update;
		async.forEach(Object.keys(update), function(item, callback){
			p[item] = update[item];
		});
	});
